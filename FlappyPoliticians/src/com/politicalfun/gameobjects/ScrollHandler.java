...
public class ScrollHandler {
	...
	private void decCredScore(int decrement) {
		gameWorld.decreaseCredibilityScore(decrement);
	}

	// Return true if ANY pipe hits the bird.
	public boolean collides(Neta neta) {
		if (!pipe1.isScored()
				&& pipe1.getX() + (pipe1.getWidth() / 2) < neta.getX()
				+ neta.getWidth()) {
			addScore(1);
			pipe1.setScored(true);
			AssetLoader.coin.play();
		} else if (!pipe2.isScored()
				&& pipe2.getX() + (pipe2.getWidth() / 2) < neta.getX()
				+ neta.getWidth()) {
			addScore(1);
			...
			AssetLoader.coin.play();
				...
				&& pipe3.getX() + (pipe3.getWidth() / 2) < neta.getX()
				+ neta.getWidth()) {
			addScore(1);
		...
		if(pipe1.touches(neta) || pipe2.collides(neta) || pipe3.collides(neta)) {
			decCredScore(1);
		}

		return (backGrass.collides(neta) || pipe1.collides(neta) || pipe2.collides(neta) || pipe3.collides(neta));
	}
